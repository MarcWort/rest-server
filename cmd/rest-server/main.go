...
func newRestServerApp() *restServerApp {
	...
	flags.BoolVar(&rv.Server.PrometheusNoPreload, "prometheus-no-preload", rv.Server.PrometheusNoPreload, "disable preloading Prometheus metrics during startup")
...
func (app *restServerApp) runRoot(_ *cobra.Command, _ []string) error {
	...
	if app.Server.Prometheus && !app.Server.PrometheusNoPreload {
		if err := app.Server.PreloadMetrics(); err != nil {
